		...
		describe( "Test Signup", function(){
            ...
            describe( "POST /signup", function(){
				...
				it( "should create a new user on successful signup", function(){
					...
					expect(	response.getData().email ).toBe( "user@example.com" );
				});
				...
				it( "should not let a user signup with an email that already exists", function(){
					...
					expect(	response.getStatusCode() ).toBe( 400 );
					expect(	response.getError() ).toBe( true );
					expect(	response.getMessages()[1] ).toBe( "An account already exists for user@example.com." );
